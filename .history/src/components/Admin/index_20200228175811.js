...
function Admin({ firebase }) {
  ...
  const [activeSprint, setActiveSprint] = useState({ id: null, active: false });
    ...
    function getActiveSprint() {
        console.log('%c 1', 'color: #bada55; font-size: 28px; padding: 10px;', 1);
        firebase.database.ref('activeSprint').on('value', value => {
            console.log('value', value.val())
            if (value && value.val().status === true) {
                setActiveSprint({ id: value.val().sprintId, active: value.val().status });
                setSelectedSprintTeam(value.val().sprintId)
            }
        })
    }
