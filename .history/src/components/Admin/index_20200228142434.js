...
function Admin({ firebase }) {
  ...
  function getSprints() {
        firebase.database.ref('sprints').once('value', (value) => {
            if (value) {
    ...
    function handleActiveSprint(event) {
        console.log('event.target.checked', event.target.value, event.target.checked);
        setActiveSprint({ sprintId: event.target.value, active: !event.target.checked });

        firebase.database.ref('activeSprint').set({sprintId: event.target.value, status: event.target.checked });
    }
