        ...
        firebase.database.ref('activeSprint').on('value', value => {
            ...
            if (value && existentActiveSprint.status === true) {
                ...
                for (const active in sprints) {
                    for (const teamId in sprints[active].teams) {
                        console.log(sprints[active].teams[teamId])
                        console.log({teamId, id: existentActiveSprint.teamId})
                        if (teamId === existentActiveSprint.teamId) {
                            console.log('aici', sprints[active].teams[teamId]);
                            setSelectedSprintTeam(sprints[active].teams[teamId].name)
                        }
                    }
                }
            }
