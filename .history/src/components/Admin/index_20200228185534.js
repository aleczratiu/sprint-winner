...
function Admin({ firebase }) {
  ...
  const [activeSprint, setActiveSprint] = useState({ id: null, active: false });
    ...
    function handleActiveSprint(event) {
        ...
        firebase.database.ref('activeSprint').set({sprintId: event.target.id || activeSprint.id, teamId: event.target.value, status: activeSprint.active });
        }
        setActiveSprint({ id: event.target.id || activeSprint.id, active: event.target.checked });
        firebase.database.ref('activeSprint').set({sprintId: event.target.id || activeSprint.id, teamId: event.target.value, status: event.target.checked || activeSprint.active });
    }
    ...
    function handleSprints() {
        ...
        for (const id in sprints) {
            console.log('selectedSprintTeam', selectedSprintTeam)
            sprintsList.push(
                <ListItem key={id}>
                    <ListItemText
