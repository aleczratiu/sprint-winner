...
function Admin({ firebase }) {
  ...
  const [activeSprint, setActiveSprint] = useState({ activeSprint: false });
  ...
  function getSprints() {
    ...
    function handleActiveSprint(event) {
        setActiveSprint(prevState => { activeSprint: !prevState.activeSprint });

        firebase.database.ref('activeSprint').set({sprintId: event.target.value, status: activeSprint });
    }
