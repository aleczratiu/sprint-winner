  ...
  function getSprints() {
    ...
    function handleActiveSprint(event) {
        if (event.target.value) {
            setActiveSprint({ activeSprint: event.target.value.split('_')[1] });
        } else {
            setActiveSprint({ activeSprint: null });
        }

        firebase.database.ref('activeSprint').set({sprintId: event.target.value.split('_')[1], status: activeSprint });
    }
