...
function Admin({ firebase }) {
  ...
  const [activeSprint, setActiveSprint] = useState({ id: null, active: false });
    ...
    }, [label, result, teamMembers, selectedTeam, sprints, selectedSprintTeam])
    ...
    function handleActiveSprint(event) {
        ...
        setActiveSprint({ id: event.target.id, active: event.target.checked || activeSprint });
        firebase.database.ref('activeSprint').set({sprintId: event.target.id, teamId: event.target.value, status: event.target.checked || activeSprint });
    }
