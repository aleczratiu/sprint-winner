...
function Admin({ firebase }) {
  ...
  const [teamName, setTeamName] = useState('');
    ...
    useEffect(() => {
        console.log({result, prevResult: prevResult.current})
        if ((!result || !prevResult.current) || Object.keys(result).every(item => !Object.keys(prevResult.current).includes(item))) {
            firebase.database.ref(label).once('value', (value) => {
                if (value) {
                    setResult(value.val());
                    prevResult.current = value.val();
                }
            });
        }
