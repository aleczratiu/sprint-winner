...
const Navbar = ({ firebase, history }) => {
    ...
    const renderMenu = (authUser, firebase) => {
        ...
        if (isAdmin) {
            return (
                <div>
                    <Button color="inherit" onClick={pushHistory(ROUTER.HOME)}>Home</Button>
                    <Button color="inherit" onClick={pushHistory(ROUTER.ADMIN)}>Admin</Button>
                ...
                <AppBar position="static">
                    <Toolbar>
                        <Typography variant="h6" className={classes.title}>
                            <a href={ROUTER.HOME} className="brand">
                            BS
                            <span>Winner</span>
                            </a>
                        </Typography>
