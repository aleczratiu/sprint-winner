...
const Winners = ({ firebase }) => {
    ...
    function getWinners() {
                ...
                for (const user in members) {
                    if (members[user].votes) {
                        for (const vote in members[user].votes) {
                            if (!f.has(members[user].name)) {
                                f.set(members[user].name, 0);
                            } else {
                                const userVote = f.get(members[user].name);
                                f.set(members[user].name, userVote++);
                            }
        ...
        console.log('setIndividualVote', result);
        // setIndividualVote(result);
